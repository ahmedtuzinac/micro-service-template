...
from models import UserService, UserServiceSchema, UserServiceCreateSchema, UserServiceUpdateSchema
...
async def get_auth_client():
    ...
    if not app_instance.auth_client:
        raise HTTPException(
            ...
            detail="Auth service nije pronađen. Proveri da li auth-service radi ili postavi AUTH_SERVICE_URL environment variable."
        )
...
async def protected_endpoint(current_user: dict = Depends(get_current_user)):
    ...
    Test sa:
    curl -H "Authorization: Bearer YOUR_JWT_TOKEN" http://localhost:8001/api/v1/user_service/protected
    """
    return {
...
async def admin_only_endpoint(admin_user: dict = Depends(require_admin)):
    ...
    curl -H "Authorization: Bearer ADMIN_JWT_TOKEN" http://localhost:8001/api/v1/user_service/admin-only
    """
    return {
...
@router.get("/", response_model=List[UserServiceSchema])
async def list_user_service(
    ...
    """
    List all user_service
    """
    query = UserService.all()
    
    if is_active is not None:
        query = query.filter(is_active=is_active)
    
    user_service = await query.offset(skip).limit(limit)
    return [UserServiceSchema.model_validate(item) for item in user_service]


@router.get("/{item_id}", response_model=UserServiceSchema)
async def get_user_service_by_id(item_id: int):
    """
    Get user_service by ID
    """
    try:
        item = await UserService.get(id=item_id)
        return UserServiceSchema.model_validate(item)
    except DoesNotExist:
        raise HTTPException(status_code=404, detail="UserService not found")


@router.post("/", response_model=UserServiceSchema, status_code=201)
async def create_user_service(item_data: UserServiceCreateSchema):
    """
    Create new user_service
    """
    item = await UserService.create(**item_data.model_dump())
    return UserServiceSchema.model_validate(item)


@router.put("/{item_id}", response_model=UserServiceSchema)
async def update_user_service(item_id: int, item_data: UserServiceUpdateSchema):
    """
    Update user_service
    """
    try:
...
async def delete_user_service(item_id: int, hard_delete: bool = Query(False)):
    """
    Delete user_service (soft delete by default)
    """
    try:
        item = await UserService.get(id=item_id)
